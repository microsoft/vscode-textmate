...
interface Peg$FilePosition {
	offset: number;
	line: number;
	column: number;
}

interface Peg$FileRange {
	start: Peg$FilePosition;
	end: Peg$FilePosition;
}

interface Peg$LiteralExpectation {
	type: 'literal';
	text: string;
	ignoreCase: boolean;
}

interface Peg$ClassParts extends Array<string | Peg$ClassParts> {}

interface Peg$ClassExpectation {
	type: 'class';
	parts: Peg$ClassParts;
	inverted: boolean;
	ignoreCase: boolean;
}

interface Peg$AnyExpectation {
	type: 'any';
}

interface Peg$EndExpectation {
	type: 'end';
}

interface Peg$OtherExpectation {
	type: 'other';
	description: string;
}

type Peg$Expectation = (
...
export class Peg$SyntaxError extends Error {
	public static buildMessage(expected: Peg$Expectation[], found: string | null) {
		...
		function describeExpectation(expectation: Peg$Expectation) {
		...
		function describeExpected(expected1: Peg$Expectation[]) {
	...
	public expected: Peg$Expectation[];
	public found: string | null;
	public location: Peg$FileRange;
	public name: string;

	constructor(message: string, expected: Peg$Expectation[], found: string | null, location: Peg$FileRange) {
		super();
...
function peg$parse(input: string, options?: Peg$ParseOptions) {
	...
	let peg$maxFailExpected: Peg$Expectation[] = [];
	...
	function location(): Peg$FileRange {
		return peg$computeLocation(peg$savedPos, peg$currPos);
	}

	function expected(description: string, location1?: Peg$FileRange) {
	...
	function error(message: string, location1?: Peg$FileRange) {
		location1 = location1 !== undefined ? location1 : peg$computeLocation(peg$savedPos, peg$currPos);

		throw peg$buildSimpleError(message, location1);
	}

	function peg$literalExpectation(text1: string, ignoreCase: boolean): Peg$LiteralExpectation {
		return { type: 'literal', text: text1, ignoreCase: ignoreCase };
	}

	function peg$classExpectation(parts: Peg$ClassParts, inverted: boolean, ignoreCase: boolean): Peg$ClassExpectation {
		return {
			type: 'class',
			parts: parts,
			inverted: inverted,
	...
	function peg$anyExpectation(): Peg$AnyExpectation {
		return { type: 'any' };
	}

	function peg$endExpectation(): Peg$EndExpectation {
		return { type: 'end' };
	}

	function peg$otherExpectation(description: string): Peg$OtherExpectation {
	...
	function peg$computeLocation(startPos: number, endPos: number): Peg$FileRange {
	...
	function peg$fail(expected1: Peg$Expectation) {
	...
	function peg$buildSimpleError(message: string, location1: Peg$FileRange) {
		return new Peg$SyntaxError(message, [], '', location1);
	}

	function peg$buildStructuredError(expected1: Peg$Expectation[], found: string | null, location1: Peg$FileRange) {
		return new Peg$SyntaxError(Peg$SyntaxError.buildMessage(expected1, found), expected1, found, location1);
	}
	...
	if (peg$result !== peg$FAILED && peg$currPos === input.length) {
		return peg$result as matchers.ParsedMatcher;
...
type Peg$ParseFunction = (input: string, options?: Peg$ParseOptions) => matchers.ParsedMatcher;

export const SyntaxError = Peg$SyntaxError;
export const parse: Peg$ParseFunction = peg$parse;
