...
tape.skip('Compare OnigLibs outputs', (t: tape.Test) => {
	...
	for (let fixturesFile of fixturesFiles) {
		...
		addTest(scopeName, testFilePath, new Registry(onigurumaResolver), new Registry(vscodeOnigurumaResolver));
	}
	t.end();
});

async function addTest(scopeName: string, filePath: string, onigurumaRegistry: Registry, vscodeOnigurumaRegistry: Registry) {
